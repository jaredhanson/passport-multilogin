...
Strategy.prototype.authenticate = function(req, options) {
  ...
  // TODO: should this be processing state at all?  or defer to passport-state
  var self = this
    , s = (req.query && req.query.select_session)
  ...
  if (s === undefined && options.multi) {
    ...
    (function iter(i) {
      ...
      function deserialized(err, user) {
        ...
        // TODO: Handle expiration and independent session lifetimes
        
        if (!user) {
          delete req.session[self._key].sessions[h];
          if (req.session[self._key].default === h) {
            delete req.session[self._key].default;
          }
        } else {
          users.push(user);
          ...
          // FIXME: only add this once, before callback, not for each user, since its a global cookie
          //.      if the data structures allow, which maybe they don't
  ...
  if (suser || suser === 0) {
    ...
    function deserialized(err, user) {
      if (err) { return self.error(err); }
      if (!user) {
        delete req.session[self._key].sessions[s];
        if (req.session[self._key].default === s) {
          delete req.session[self._key].default;
        }
      } else {
        var property = req._userProperty || 'user';
