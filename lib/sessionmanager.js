  ...
  this._serializeUser(user, req, function(err, user) {
    ...
    for (i = 0, len = handles.length; i < len; ++i) {
      ...
      if (suser && utils.equalsUser(suser, user)) {
        ...
        var events = req.session[self._key][h].events
          , j, jlen;
        for (j = 0, jlen = events.length; i < len; ++i) {
          if (utils.equalsMethod(events[j], event)) {
            req.session[self._key][h].events[j] = event;
            return cb();
          }
        }
    ...
    req.session[self._key][h] = { user: user, events: [] };
    req.session[self._key][h].events.push(event);
