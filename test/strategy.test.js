...
describe('Strategy', function() {
  ...
  it('should pass request with one login session using multi option and set user to default session using verify function that accepts user and callback', function(done) {
    var strategy = new Strategy(function(user, cb) {
      expect(user).to.deep.equal({ id: '248289761001', displayName: 'Jane Doe' });
      cb(null, user);
    });
    ...
    chai.passport.use(strategy)
      ...
      .authenticate({ multi: true });
  }); // should pass request with one login session using multi option and set user to default session using verify function that accepts user and callback
  ...
  it('should remove secondary session when login session has been invalidated using multi option', function(done) {
    ...
    verify.onCall(1).yields(null, false);
    var strategy = new Strategy(verify);
  ...
  it('should error when verify function encounters an error using multi option', function(done) {
    var strategy = new Strategy(function(req, user, cb) {
      cb(new Error('something went wrong'));
    });
    ...
    chai.passport.use(strategy)
      ...
      .authenticate({ multi: true });
  }); // should error when verify function encounters an error using multi option
